        ...
        if (entry.isIntersecting) {
          ...
          if (viewTitle.textContent !== activeTitleName) {
            const directionClass =
              activeTitleName === "Tasks" ? "slide-left" : "slide-right";
...
export function addEditTaskListener(card, { setBackgroundColor }) {
  card.addEventListener("change", (event) => {
    if (
      event.target.classList.contains("checklist-input") ||
...
export function addEditPriorityListener(priorityBtn, priorityColourMap) {
  ...
  priorityBtn.addEventListener("click", (event) => {
    ...
    if (currTask) {
      const currOptionIndex = priorityColourMap.findIndex(
        (option) => option.priority === currTask.priority,
      );
...
export function addEditChecklistListener(
  checklistUl,
...
export function addDeleteProjListener(
  deleteProjBtn,
  projCard,
...
export function addEditProjListener(
  card,
  { setBackgroundColor, renderFilterMenuOptions },
) {
  card.addEventListener("change", (event) => {
    event.preventDefault();
